# Questions à poser à l'artisan

459 informations manquent sur le site. Elles sont repérées dans le code source, mais masquées à l'écran pour la présentation. Le fichier `A-COMPLETER.md` explique comment les réafficher.

Ce document les regroupe en questions concrètes. Les sections **1 à 4** empêchent la mise en ligne. Les sections **5 à 10** enrichissent le site sans le bloquer.

Rien n'a été inventé. Aucun chiffre, avis, certification, durée ou prix ne figure sur le site sans avoir été fourni.

---

## 1. Identité légale, trois contradictions à lever

L'attestation INPI du 31 juillet 2026 a réglé la forme juridique, le directeur de la publication et l'immatriculation. Elle soulève en revanche trois points que **vous seul pouvez trancher**, et qui sont bloquants.

1. **L'activité déclarée est le nettoyage courant des bâtiments, code APE 8121Z.** Le site présente vingt-huit pages d'élagage et de soins aux arbres. L'élagage relève normalement du code 8130Z, services d'aménagement paysager. Avez-vous une seconde activité déclarée, ou le code doit-il être modifié auprès du guichet des formalités ?
2. **L'attestation indique un début d'activité au 1er août 2026.** Le site annonce « élagueur depuis 2006 », plus de 220 chantiers par an et plus de 160 interventions depuis janvier. Votre SIREN date bien de 2006 et l'attestation mentionne une reprise de données, donc l'ancienneté est plausible. Confirmez-vous les vingt années d'exercice, et sous quelle structure avant août 2026 ? D'où viennent les chiffres de 220 et 160 ?
3. **Le nom commercial déposé est « LCC », pas « LCC Espaces Verts ».** Souhaitez-vous déclarer le nom complet, ou ajuster l'affichage du site ?

Déjà en place et vérifié : Cédric Larme, entrepreneur individuel, SIREN 490 985 520, SIRET 490 985 520 00033, siège 1 avenue de Bellevue à Mérignac, immatriculation au registre national des entreprises le 31 juillet 2026.

Restent à fournir : le numéro de TVA intracommunautaire si vous y êtes assujetti, et le médiateur de la consommation retenu.

## 2. Assurance et certifications, 47 emplacements

Bloquant. Un prospect qui ne voit ni assurance ni certification passe au concurrent suivant.

4. Quel est le nom de l'assureur en responsabilité civile professionnelle ? *(40 emplacements)*
5. Quel est le numéro du contrat ?
6. Que couvre exactement la garantie, et à quelle hauteur ?
7. Quelles certifications d'élagueur grimpeur l'équipe détient-elle, CS taille et soins aux arbres, CQP, autre ?
8. Y a-t-il une autorisation de conduite de nacelle, et pour quelle hauteur de travail ?
9. Y a-t-il une habilitation électrique pour intervenir près des réseaux ?
10. Quelles autorisations de conduite d'engins pour le broyage et la rogneuse ?
11. Quelles habilitations pour le travail en hauteur ?
12. Existe-t-il une qualification en diagnostic de l'arbre, visuelle ou instrumentée ?

## 3. Coordonnées, 54 emplacements

Bloquant. Le formulaire de devis et les mentions légales en dépendent.

13. Quelle adresse e-mail afficher publiquement ? *(53 emplacements)*
14. Faut-il une adresse e-mail distincte pour les demandes liées aux données personnelles ?

Déjà en place : 06 41 59 80 54, du lundi au samedi de 7h30 à 19h.

## 4. Hébergeur du site, 4 emplacements

Bloquant. Obligation légale sur les mentions légales.

L'hébergeur est désormais connu, c'est **Vercel**. Restent à reprendre depuis ses conditions officielles, sans les inventer :

15. La raison sociale exacte de l'entité qui héberge, et son adresse complète.
16. Son numéro de téléphone ou, à défaut, son moyen de contact officiel.
17. Les mesures de sécurité qu'il annonce, pour la page de confidentialité.
18. Les données transitant hors de l'Union européenne, il faut le mentionner dans la politique de confidentialité et indiquer le cadre juridique du transfert.

---

## 5. Chantiers présentés, 169 emplacements

C'est le plus gros volume. Chaque chantier montré sur le site attend les mêmes champs.

En attendant vos chantiers, **tout ce qui les présente est masqué**, y compris la page réalisations. Une galerie de cadres vides dessert plus qu'elle ne montre. Rien n'a été supprimé, tout revient d'un seul réglage le jour où vous fournissez les informations ci-dessous.

Sur l'accueil, la page réalisations, les 9 pages service et les 12 pages ville, il faut pour chaque chantier :

| Champ | Nombre d'emplacements |
| --- | --- |
| Durée du chantier | 56 |
| Essence de l'arbre | 33 |
| Commune | 33 |
| Hauteur de l'arbre | 31 |
| Quartier | 22 |
| Linéaire de haie | 4 |
| Surface traitée | 3 |
| Diamètre du tronc | 3 |
| Volume de déchets verts | 2 |
| Autres, longueur, profondeur de rognage, nombre d'arbres | 6 |

19. Pouvez-vous détailler une dizaine de chantiers réels, avec pour chacun la commune, le quartier, l'essence, la hauteur ou le linéaire, et la durée ?

**Le plus simple** : reprenez dix chantiers récents dans votre agenda et donnez ces cinq champs pour chacun. Les textes autour sont déjà écrits.

## 6. Photographies

Des photos de banque libres de droits ont été posées pour la présentation, sur toutes les pages. Chacune porte la mention visible « Photo d'illustration ». Aucune ne montre un chantier de l'entreprise.

Certaines ne collent pas tout à fait au texte qu'elles accompagnent, et sont donc à remplacer en priorité.

La plus gênante est celle de la page zones d'intervention : c'est un grumier chargé de rondins, qui laisse croire à un matériel de débardage plus lourd que le vôtre. Une photo de votre camion et de votre remorque la remplacerait avantageusement.

Viennent ensuite la page abattage, où l'opérateur travaille sans corde de traction alors que le texte décrit ce matériel, les pages haies et dessouchage, où la machine est montrée seule sans opérateur, et le bandeau de la page entreprise, qui montre un ramassage de feuilles plutôt qu'un chantier d'élagage.

20. Pouvez-vous fournir vos propres photos de chantier ?
21. Avez-vous des paires avant et après ? C'est ce qui convainc le mieux, et cela ne peut pas être remplacé par une photo d'illustration.
22. Avez-vous l'accord écrit des propriétaires pour publier ces photos ?
23. Existe-t-il un modèle d'autorisation de publication signé par les clients ?
24. Avez-vous une photo du gérant en intervention, harnais visible, et une photo d'équipe devant le camion ?

Format attendu, WebP ou JPEG, 1600 pixels de large minimum.

## 7. Avis clients

25. Quelle est la note Google et le nombre d'avis ?
26. Pouvez-vous transmettre trois avis, à reprendre mot pour mot depuis la fiche Google Business, avec le prénom de leur auteur et la commune ?

Un avis reformulé ou inventé est un risque juridique. Il faut le texte exact.

## 8. Zone d'intervention et délais

27. Quel est le rayon d'intervention en kilomètres ?
28. Quelles communes de périphérie traiter en priorité ?
29. Quel rayon maximal acceptez-vous au-delà de la métropole ?
30. Quel délai d'intervention pouvez-vous annoncer en urgence après tempête ?
31. Intervenez-vous en dehors des horaires, la nuit et le dimanche ?
32. Quel est le délai moyen de rappel après une demande ?
33. Quelle est la durée de validité d'un devis ?
34. Recevez-vous sur place, et sur quels créneaux ?

## 9. Matériel et méthodes

Une question par page service. Le visiteur qui compare deux devis regarde le matériel.

35. **Élagage** : cordes de rétention et charge admissible, poulies, descendeur, nacelle et hauteur de travail, broyeur, benne ?
36. **Abattage** : tronçonneuses et longueurs de guide, treuil, cordes de traction, coins, broyeur, benne ?
37. **Démontage** : nacelle et hauteur, tronçonneuses, treuil, moyens de rétention à la corde ?
38. **Dessouchage** : modèle de rogneuse, largeur de passage, mini-pelle, moyens d'évacuation, profondeur de rognage atteinte ?
39. **Haies** : taille-haies thermiques ou batterie, perche télescopique, échafaudage roulant, broyeur ?
40. **Débroussaillage** : débroussailleuses portées, type de broyeur, engin porteur, largeur de travail ?
41. **Évacuation** : modèle de broyeur, diamètre maximal de branche admis, benne ou remorque, fendeuse ?
42. **Manutention** : diable, brouette motorisée, planches de roulement, charge maximale, nombre de personnes mobilisables ?
43. **Évacuation, suite** : quelle filière de dépôt utilisez-vous, et quel volume maximal transportable ?
44. Disposez-vous d'une nacelle en propre ou la louez-vous ?
45. Combien de personnes composent l'équipe ?
46. Quel est le parcours du gérant et quelle formation a-t-il suivie ?
47. Avez-vous des références professionnelles ou des marchés publics à citer ?

## 10. Prix

48. Acceptez-vous d'afficher des fourchettes indicatives ? Sur quelles prestations ?
49. Y a-t-il une majoration en cas d'urgence ?

Si vous préférez ne pas afficher de prix, les mentions correspondantes seront simplement retirées. C'est un choix commercial, pas un défaut.

## 11. Démarches administratives

50. Vous chargez-vous des déclarations en mairie, ou laissez-vous le client s'en occuper ?

## 12. Urbanisme, une question par commune

Le site annonce des règles locales sur les 12 pages ville. Chaque affirmation doit être vérifiée avant publication, sinon elle engage l'entreprise.

51. Pouvez-vous confirmer, ou faut-il que je vérifie auprès de chaque service urbanisme, les points suivants ?

| Commune | Point à vérifier |
| --- | --- |
| Mérignac | Périmètre exact des servitudes aéronautiques, arbres protégés au PLU |
| Bordeaux | Périmètre du site patrimonial remarquable, rues concernées, délai d'autorisation de voirie |
| Pessac | Secteur protégé autour de la Cité Frugès, arbres classés au PLU |
| Talence | Arbres remarquables et alignements protégés au PLU |
| Gradignan | Règlement applicable aux berges de l'Eau Bourde, interlocuteur compétent |
| Villenave-d'Ornon | Nature du sol et emprise des secteurs alluviaux |
| Le Bouscat | Arbres et alignements protégés au PLU intercommunal |
| Bruges | Périmètre de la réserve naturelle des marais, règles pour les parcelles riveraines |
| Blanquefort | Arbres et alignements protégés au PLU intercommunal |
| Eysines | Règles d'entretien des berges des jalles, parcelles protégées |
| Le Haillan | Servitudes aéronautiques, règles de mitoyenneté |
| Saint-Médard-en-Jalles | Obligation légale de débroussaillement, périmètre et contenu, règles de brûlage |

52. Quelles communes sont soumises à une obligation légale de débroussaillement ? À vérifier en préfecture.
53. Y a-t-il des usages locaux particuliers sur vos communes d'intervention ?
54. Les coordonnées de chaque service urbanisme sont à récupérer, 12 emplacements.

## 13. Données personnelles et médiation

55. Quelle durée de conservation retenez-vous pour les demandes sans suite, douze ou vingt-quatre mois ?
56. Quels outils utilisez-vous, hébergeur, messagerie, gestion des devis, et où sont-ils localisés ?
57. Souhaitez-vous un outil de statistiques ? Cela impose une information complémentaire au visiteur.
58. Quel médiateur de la consommation avez-vous retenu, nom et coordonnées ? Obligation légale pour les particuliers.

---

## Nom de domaine

Réglé. Le domaine est `lcc-espacesverts.fr` et le site est en ligne. L'adresse de référence est `www.lcc-espacesverts.fr`, l'adresse sans `www` y redirige automatiquement.

Le site est ouvert aux moteurs de recherche. Google peut donc l'indexer et le faire apparaître dans ses résultats dans les jours qui viennent.

**Cela rend les sections 1 à 4 urgentes.** Les mentions légales sont publiques alors qu'il y manque l'hébergeur et l'adresse e-mail, deux informations obligatoires. Un site qui vend de l'élagage sous un code d'activité de nettoyage de bâtiments est également exposé. Ces points étaient à régler avant mise en ligne, ils sont maintenant à régler au plus vite.

---

## Les trois priorités

Si l'artisan n'a le temps de répondre qu'à une partie :

1. **Le code APE, l'ancienneté, l'assureur et l'adresse e-mail.** Sans cela le site ne peut pas être mis en ligne.
2. **Dix chantiers détaillés avec photos avant et après.** C'est ce qui fait la différence face à un concurrent.
3. **Note Google et trois avis repris mot pour mot.**
